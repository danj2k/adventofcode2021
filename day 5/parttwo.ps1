...
foreach ($line in $inputs) {
	...
	# add line to be considered - we're including diagonals
	$inputlines += $thisline
}
$biggestx++
$biggesty++
...
foreach ($line in $inputlines) {
	if ($line.x1 -eq $line.x2) {
		...
		do {
			$map[$x,$y]++
			if ($line.x2 -gt $line.x1) { $x++ }
			if ($line.x2 -lt $line.x1) { $x-- }
			if ($line.y2 -gt $line.y1) { $y++ }
			if ($line.y2 -lt $line.y1) { $y-- }
		} until ($x -eq $line.x2 -and $y -eq $line.y2)
		$map[$x,$y]++
	}
}
